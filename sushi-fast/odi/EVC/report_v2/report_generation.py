...
from .report_aggregation import build_reaction_trace, build_report_evidence
from .report_metric_catalog import DETAIL_METRIC_SPECS
...
_INSIGHT_CACHE: OrderedDict[str, ReportNarrativeOutput | AIInsight] = OrderedDict()
...
def _insight_cache_key(payload: dict[str, Any], provider: object) -> str:
    ...
    return hashlib.sha256(f"presentation-report-v2\n{model}\n{normalized}".encode("utf-8")).hexdigest()


class ReportInsightProvider(Protocol):
    def generate(self, payload: dict[str, Any]) -> ReportNarrativeOutput | AIInsight: ...
...
class OpenAIReportInsightProvider:
    ...
    def generate(self, payload: dict[str, Any]) -> ReportNarrativeOutput:
        if not self.api_key:
        ...
        try:
            completion = OpenAI(api_key=self.api_key).chat.completions.parse(
                ...
                messages=[
                    {
                        ...
                        "content": (
                            ...
                            "ground every claim in the transcript/segment evidence, and return a concise "
                            "Korean title, description, strengths, improvements, and metric_narratives. "
                            "Return one metric narrative for every supplied metric id. Each narrative must include "
                            "a concise reason, a concrete coaching action, and only supplied evidence_ids. "
                            "Never return scores, rank labels, status labels, or display labels. "
                            "Every strength or improvement "
                            "must cite only evidence_ids supplied by the user and include one concrete next action. "
                            "When qa_history is present, also assess the answers, "
                ...
                response_format=ReportNarrativeOutput,
            )
...
def build_report_insight_payload(
    ...
    evidence_by_step = {item.source_step: item for item in build_report_evidence(evidence)}
    ...
    for item in evidence:
        ...
        serialized_segments.append(
            {
                "evidence_id": f"segment-{item.step}",
                "step": item.step,
                ...
                "metric_impacts": evidence_by_step[item.step].metric_impacts,
                "missing_inputs": item.evaluation.missing_inputs,
            }
        )
                ...
                "audience_actions": [
                    {
                        ...
                        "behavior": (
                            decision.core_behavior.behavior_id
                            if decision.core_behavior is not None else None
                        ),
                        "direction": decision.direction,
                        "previous_state": decision.previous_state.model_dump(),
                        "state": decision.state.model_dump(),
                    }
                    for decision in item.audiences
                ],
                "unity_commands": [
                    {
                        ...
                        "behavior_id": command.selected_behavior_id,
                    }
                    for command in item.commands
                ],
            }
            for item in reaction_trace
        ],
    }
...
) -> ReportNarrativeOutput | AIInsight:
    ...
    for attempt in range(retries + 1):
        try:
            result = await asyncio.wait_for(
                asyncio.to_thread(selected.generate, payload), timeout=timeout_s
            )
