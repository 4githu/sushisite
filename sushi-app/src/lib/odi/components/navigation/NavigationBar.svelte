...
<script lang="ts">
	...
	const currentPage = $derived.by(() => {
		...
		if (pathname.startsWith('/odi/templates')) return 'favorites';
	...
	function trackNavigation(node: HTMLElement) {
		navigation = node;
	}
	onMount(() => {
...
<aside class="navigation" {@attach trackNavigation}>
	<button
		type="button"
	...
	<div class="top">
		...
		<div class="menu">
			...
			<NavigationItem
				...
				onclick={() => navigateTo('/odi/templates')}
			/>
			<NavigationItem
				label="Report"
				icon={Report}
