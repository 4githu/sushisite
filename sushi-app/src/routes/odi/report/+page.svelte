<script lang="ts">
	...
	import {
		...
		scoreGrade
	} from '$lib/odi/components/report/reportUtils';
...
<main class="report-list-page">
	...
	<section class="summary-grid" aria-label="리포트 통계">
		<article class="stat-card score">
			...
			<b>{summary.average}<em>점</em></b>
		</article>
		<article class="stat-card time">
			<div>
				<span>♨</span>
				<div><strong>총 연습 시간</strong><small>완료된 세션 기준</small></div>
			</div>
			<b>{formatKoreanDuration(summary.totalSeconds)}</b>
		</article>
	</section>

	<section class="report-list">
		<div class="toolbar">
			<select bind:value={sortOrder} aria-label="정렬"
			...
			<div class="rows">
				{#each completedSessions as item (item.session_id)}
					<article class="report-row">
						<div class="type">
							<img src={iconFor(item)} alt="" /><span
								>{item.template?.type === 'interview' ? '면접' : '발표'}</span
							>
						</div>
						<div class="info">
							<strong>{titleFor(item)}</strong><small
								>{detailsFor(item)} · {audienceFor(item)}</small
							>
						</div>
						<div>{formatKoreanDuration(durationFor(item))}</div>
						<div class="score-value">
							<strong>{scoreFor(item)}</strong><small
						...
						<div class="actions">
							<button type="button" onclick={() => goto(`/odi/report/${item.session_id}`)}
								>자세히 보기</button
							><button
								class="delete"
								type="button"
								...
								>{deletingId === item.session_id ? '삭제 중' : '삭제'}</button
							>
						</div>
					</article>
				{/each}
...
<style>
	...
	.report-row {
		display: grid;
		grid-template-columns: 86px minmax(250px, 1.6fr) 100px 90px minmax(180px, 1.2fr) 145px 165px;
		gap: 18px;
		align-items: center;
	}
	.table-head {
		padding: 16px 14px;
		border-bottom: 2px solid var(--cool-grey-light-active);
		color: var(--text-secondary);
		font-size: 13px;
		font-weight: 700;
	}
	.report-row {
		padding: 20px 14px;
		border-bottom: 1px solid var(--cool-grey-light-active);
		font-size: 14px;
	}
	.type {
		display: flex;
