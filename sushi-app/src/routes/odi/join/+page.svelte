...
<script lang="ts">
	...
	const canFinishGoal = $derived(
		Boolean(trainingType && focusArea && practiceFrequency) && !loading
	);
...
{:else}
	<main class="join-page">
		<header class="page-header">
			<div class="title-group">
				<h1 class="text-title-main">
					{currentStep === 0 ? 'Re:hear 시작하기' : '훈련 목표 설정하기'}
				</h1>
				<p class="subtitle text-caption-main">
					{currentStep === 0
						? '현재 로그인된 계정을 확인하고 Re:hear를 시작해보세요'
						: '훈련 목표를 설정하고 나에게 맞는 미션을 제공받으세요'}
				</p>
			</div>
		</header>

		<AuthStepper {steps} {currentStep} />
		...
		<section class="join-content">
			...
			{:else}
				<OdiJoinGoalStep bind:trainingType bind:focusArea bind:practiceFrequency />
			{/if}
		</section>

		<footer class="page-actions">
			<Button variant="secondary" width="360px" onclick={goPrevious}>이전 단계</Button>

			{#if currentStep === 0}
				<Button
					variant="primary"
					width="360px"
					disabled={!canNextFromAccount}
