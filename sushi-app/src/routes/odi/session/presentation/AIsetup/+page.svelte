...
<script lang="ts">
	...
	onMount(() => {
		const draft = getPresentationDraft();
		if (draft === null) {
	...
	const canNext = $derived(personaType !== '' && expertiseLevel >= 1 && interestLevel >= 1);

	async function goNext() {
		if (isSaving || !template.get()?.files.slide?.storage_path) {
	...
	<header class="page-header">
		...
		<div class="title-group">
			...
			<p class="subtitle text-caption-main">
				청중 페르소나를 설정하여 원하는 발표 분위기를 구성할 수 있어요.
			</p>
		</div>
	</header>
		...
		<TipCard
			...
			tips={[
				...
				},
				{
					icon: Check,
					description:
						'청중 규모와 분위기에 따라 발표 몰입도와 대응력을 효과적으로 훈련할 수 있습니다.'
				}
			]}
		/>
	</section>
